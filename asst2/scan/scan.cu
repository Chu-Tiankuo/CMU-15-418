...
__global__ void upsweep_kernel(int*data,int N,int twod){
    ...
    int i=idx*twod1;
    if(i<N)data[i+twod1-1]+=data[i+twod-1];
...
void exclusive_scan(int* device_start, int length, int* device_result)
{
    ...
    for(int twod=1;twod<rounded_length/2;twod*=2){
    	int twod1=twod*2;
	int threads=rounded_length/twod1;
	int threadsPerBlock=256;
	int blocks=(threads+threadsPerBlock-1)/threadsPerBlock;
	upsweep_kernel<<<blocks,threadsPerBlock>>>(device_result,rounded_length,twod);
	...
	int threads=rounded_length/twod1;
        int threadsPerBlock=256;
        int blocks=(threads+threadsPerBlock-1)/threadsPerBlock;
	downsweep_kernel<<<blocks,threadsPerBlock>>>(device_result,rounded_length,twod);
	cudaDeviceSynchronize();
    }
...
__global__ void flags_kernel(int*input,int*flags,int length){
    int idx=blockIdx.x*blockDim.x+threadIdx.x;
    if(idx<length-1)flags[idx]=input[idx]==input[idx+1]?1:0;
    else if(idx==length-1)flags[idx]=0;
}
__global__ void compact_kernel(int*flags,int*scan,int*output,int length){
    int idx=blockIdx.x*blockDim.x+threadIdx.x;
    if(idx<length&&flags[idx]==1)output[scan[idx]]=idx;
}
int find_repeats(int *device_input, int length, int *device_output) {
    ...
    cudaMalloc(&device_scan,rounded_length*sizeof(int));

    int threadsPerBlock=256;
    int blocks=(length+threadsPerBlock-1)/threadsPerBlock;
    flags_kernel<<<blocks,threadsPerBlock>>>(device_input,device_flags,length);
    cudaDeviceSynchronize();

    cudaMemcpy(device_scan,device_flags,length*sizeof(int),cudaMemcpyDeviceToDevice);
    exclusive_scan(device_flags,length,device_scan);
    
    compact_kernel<<<blocks,threadsPerBlock>>>(device_flags,device_scan,device_output,length);
    cudaDeviceSynchronize();
